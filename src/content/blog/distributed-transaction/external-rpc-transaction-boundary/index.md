...
flowchart TB
    ...
    subgraph S1["方案一：RPC 在本地事务之前"]
        direction LR
        S1A[调用 RPC] --> S1B{RPC 结果}
        S1B -->|明确失败| S1C[返回失败<br/>本地事务不启动]
        S1B -->|明确成功| S1D[开启本地事务]
        S1D -->|提交成功| S1E[返回成功]
        S1D -->|提交失败| S1F[远程成功、本地失败<br/>补偿或人工处理]
        S1B -->|结果未知| S1G[不能安全继续<br/>查询、补偿或人工处理]
    end

    subgraph S2["方案二：RPC 在本地事务内部"]
        direction LR
        S2A[开启本地事务] --> S2B[写入本地数据]
        S2B --> S2C[调用 RPC]
        S2C --> S2D{RPC 结果}
        S2D -->|明确失败| S2E[回滚本地事务<br/>返回失败]
        S2D -->|明确成功| S2F[尝试提交本地事务]
        S2F -->|提交成功| S2G[返回成功]
        S2F -->|提交失败| S2H[远程成功、本地失败<br/>补偿或人工处理]
        S2D -->|结果未知| S2I[本地事务无法等待人工<br/>回滚或记录待确认状态]
    end

    subgraph S3["方案三：本地事务提交后直接调用 RPC"]
        direction LR
        S3A[开启本地事务] --> S3B[写入本地数据]
        S3B --> S3C[提交本地事务]
        S3C -->|提交失败| S3D[返回本地失败<br/>RPC 未调用]
        S3C -->|提交成功| S3E[调用 RPC]
        S3E --> S3F{RPC 结果}
        S3F -->|明确成功| S3G[返回成功]
        S3F -->|明确失败| S3H[本地已提交<br/>执行补偿；补偿失败则待处理]
        S3F -->|结果未知| S3I[本地已提交、远程状态未知<br/>查询、补偿或人工处理]
        S3C -.进程在调用 RPC 前崩溃.-> S3J[本地已提交<br/>远程未调用]
    end

    subgraph S4["方案四：本地事务 + Outbox/任务表"]
        direction LR
        S4A[开启本地事务] --> S4B[写入本地数据<br/>写入 Outbox 任务]
        S4B --> S4C[提交本地事务]
        S4C -->|提交失败| S4D[业务数据和任务一起回滚]
        S4C -->|提交成功| S4E[返回已受理/处理中]
        S4E --> S4F[任务执行器调用 RPC]
        S4F --> S4G{RPC 结果}
        S4G -->|明确成功| S4H[任务完成<br/>更新业务状态]
        S4G -->|明确失败| S4I[重试或执行补偿]
        S4G -->|结果未知| S4J[使用幂等键重试<br/>查询或人工处理]
        S4C -.进程崩溃.-> S4K[任务仍在 Outbox 中<br/>后续可以继续执行]
    end
